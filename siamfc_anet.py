...
class SiamFC(nn.Module):
    ...
    def __init__(self):
        ...
        self.SEblock = ChannelAttention(256)
        mod = torch.load('alexnet_vid.pth')
        for i in range(len(self.feature.state_dict().items())):
            list(self.feature.state_dict().items())[i][1].data[:] = list(mod.items())[i][1].data[:]
...
class TrackerSiamFC(Tracker):
    ...
    def init(self, image, box):
        ...
        with torch.set_grad_enabled(False):
            ...
            self.kernel = self.kernel * self.net.SEblock(self.kernel)
